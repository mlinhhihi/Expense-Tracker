...
* **Xác thực trực tiếp qua Firebase:** Sử dụng API `createUserWithEmailAndPassword` để xử lý đăng ký bất đồng bộ. Ngay sau khi Firebase Authentication cấp tài khoản thành công, ứng dụng sẽ thông báo và tự động giải phóng màn hình (`finish()`) để đưa người dùng quay lại giao diện đăng nhập.
...
* **Xử lý ảnh đại diện nâng cao (Base64 Processing):** * Tích hợp bộ chọn ảnh đời mới `PickVisualMedia` để truy cập an toàn vào thư viện thiết bị.
  * Tự động nén chất lượng (`Bitmap.compress` ở mức 50%) và mã hóa thực thể hình ảnh sang chuỗi văn bản **Base64** giúp đẩy trực tiếp lên Firestore một cách gọn nhẹ mà không cần qua dịch vụ lưu trữ file cồng kềnh.
...
* **Tối ưu hóa UI/UX Biểu đồ Cao cấp (MPAndroidChart Customization):** * Định dạng hiển thị dữ liệu thuần ở dạng tỷ lệ phần trăm (`setUsePercentValues`) kết hợp hiệu ứng hoạt họa quét góc `animateY(1000)` mượt mà.
